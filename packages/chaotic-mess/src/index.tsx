import planck, {testbed, Box, Circle, Edge, Polygon, RevoluteJoint, Vec2, WheelJoint, World } from "planck/dist/planck-with-testbed"
...
testbed(function(testbed) {
  ...
  // Bridge
  let bridgeFD = {density: 1.0, friction: 0.6};
  ...
  for (var i = 0; i < 20; ++i) {
    ...
    world.createJoint(RevoluteJoint({}, prevBody, bridgeBlock, Vec2(160.0 + 2.0 * i, -0.125)));

    prevBody = bridgeBlock;
  }

  world.createJoint(RevoluteJoint({}, prevBody, ground, Vec2(160.0 + 2.0 * i, -0.125)));
  ...
  var wheelFD = {density: 1.0, friction: 0.9};
  ...
  var springBack = world.createJoint(WheelJoint({
    motorSpeed : 0.0,
    maxMotorTorque : 20.0,
    enableMotor : true,
    frequencyHz : HZ,
  ...
  var springFront = world.createJoint(WheelJoint({
    motorSpeed : 0.0,
    maxMotorTorque : 10.0,
    enableMotor : false,
    frequencyHz : HZ,
